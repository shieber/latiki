...
class FieldStorage:
    ...
    def __init__(self, fp=None, headers=None, outerboundary="",
        ...
        if ctype == 'application/x-www-form-urlencoded':
            ...
            # mod by jdb
            if (path_info != None and path_info.find("/repo/") == 0 \
                and headers.has_key('Content-Type') \
                and headers['Content-Type'].find("text/plain") == 0) \
            or (headers.get("User-Agent","").find('rv:1.7.12') > -1 and 
                headers.get("Remote-Addr","") == "127.0.0.1"):
                # FIXME: GWT doesn't allow you to modify the headers
                # of a POST.  so we kludge it.  note that hosted mode
                # doesn't send a refereer (gack), so we also kluge it
                # this is fail. we should fix GWT.
                self.read_urlencoded() 
            else:                
                self.read_single()
